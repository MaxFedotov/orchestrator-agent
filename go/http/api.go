...
// GetMySQLInfo returns basic MySQL info to be used in pre-seed checks
func (this *HttpAPI) GetMySQLInfo(params martini.Params, r render.Render, req *http.Request) {
	if err := this.validateToken(r, req); err != nil {
		return
	}
...
func (this *HttpAPI) DeleteMySQLDataDir(params martini.Params, r render.Render, req *http.Request) {
	...
	err := osagent.DeleteDirContents(config.Config.MySQLDataDir)
	if err != nil {
		r.JSON(500, &APIResponse{Code: ERROR, Message: err.Error()})
		return
	}
...
func (this *HttpAPI) DeleteMySQLBackupDir(params martini.Params, r render.Render, req *http.Request) {
	if err := this.validateToken(r, req); err != nil {
		return
	}
	err := osagent.DeleteDirContents(config.Config.MySQLBackupDir)
	if err != nil {
		r.JSON(500, &APIResponse{Code: ERROR, Message: err.Error()})
		return
	}
